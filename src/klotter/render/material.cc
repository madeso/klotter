...
)
{
	...
	for (int index = 0; index < settings.number_of_point_lights; index += 1)
	{
		...
		shader.program->set_vec4(u.light_attenuation_uni, {p.min_range, p.max_range, p.curve.curve.s, p.curve.curve.t});
	}
	...
	for (int index = 0; index < settings.number_of_frustum_lights; index += 1)
	{
		...
		shader.program->set_vec4(u.attenuation_uni, {p.min_range, p.max_range, p.curve.curve.s, p.curve.curve.t});
