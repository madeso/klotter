...
struct SceneApp : App
{
    ...
    MeshInstancePtr add_cube(float x, float y, float z, const glm::vec3& color)
    {
        const auto triangle = mesh::create_box(x, y, z, false, color).to_mesh();
    ...
    void add_cube(const glm::vec3& p, int index)
    {
        auto cube = add_cube(1.0f, 1.0f, 1.0f, colors::white);
        cube->position = p;
    ...
    SceneApp()
    {
        ...
        add_cube({ 0.0f,  0.0f,   0.0f}, 0);
        add_cube({ 2.0f,  5.0f, -15.0f}, 1);
        add_cube({-1.5f, -2.2f,  -2.5f}, 2);
        add_cube({-3.8f, -2.0f, -12.3f}, 3);
        add_cube({ 2.4f, -0.4f,  -3.5f}, 4);
        add_cube({-1.7f,  3.0f,  -7.5f}, 5);
        add_cube({ 1.3f, -2.0f,  -2.5f}, 6);
        add_cube({ 1.5f,  2.0f,  -2.5f}, 7);
        add_cube({ 1.5f,  0.2f,  -1.5f}, 8);
        add_cube({-1.3f,  1.0f,  -1.5f}, 9);
    }
